...
export const DataContext = createContext<UserContextType>({} as UserContextType);
...
function UserContext({ children }: { children: React.ReactNode }) {
  ...
  async function aiResponseHandler(prompt: string) {
    try {
      ...
      if (prompt.toLowerCase().includes("stop") || 
        ...
        stopSpeaking();
        const stopMessage = "I've stopped speaking as requested.";
        setAiResponse(stopMessage);
        setChatHistory(prev => [...prev, { role: 'assistant', content: stopMessage }]);
        return;
      }
  ...
  const prepareInterviewQuestionsFromPdf = async (): Promise<string[]> => {
    ...
    try {
      const questions = await prepareInterviewQuestions(pdfContent);
      setInterviewQuestions(questions);
      toast.success("Interview questions prepared", { description: `${questions.length} questions generated` });
      return questions;
    } catch (error: any) {
